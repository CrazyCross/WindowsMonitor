...
def judgeIfAlive(ProgramPath,ProcessName):
    ...
    #日志文件名是以年月日为文件名
        ...
        print ProcessName+' Server is not running,Begining to Restart Server...' 
    #打印服务状态
    ...
    #调用服务重启
        f.write( ProcessName+' Restart Server Success...'+'\n') 
        f.write(time.strftime('%Y-%m-%d %H:%M:%S --%A--%c', time.localtime())) 
        f.close() 
    #关闭文件
...
def main(ProgramPathDict,ProcessNameDict) : 
    while True:
...
if __name__=="__main__" : 
    ...
    main(ProgramPathDict,ProcessNameDict) 
